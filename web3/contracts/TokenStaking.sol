...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
  //Struct to store the Uer's Details
  struct User {
  ...
  uint256 _stakeStartDate;  //start date fro program
  ...
  uint256 _earlyUnstakeFeePercentage; // early unstake fee percentage
   ...
   address private _tokenAddress;   //Token conctract address
   ...
   modifier whenTreasuryHasBalance(uint256 amount) {
    require(
      ...
      "TokenStaking: insuficient funds in the treasury"
    );
    _;
   }
  ...
  uint256 earlyUnstakeFeePercentage_
    ...
    __TokenStaking_init_unchained(owner_, tokenAddress_, apyRate_, minimumStakingAmount_, maxStakeTokenLimit_, stakeStartDate_, stakeEndDate_, stakeDays_, earlyUnstakeFeePercentage_);
   }
  ...
  uint256 earlyUnstakeFeePercentage_
    ...
    require(stakeDays_ > 0, "TokenStaking: sake days must be non-zero");
...
function getMinimumStakingAmount() external view returns (uint256) { 
  ...
  function toggleSakingStatus( ) external onlyOwner {
...
_totalStakedTokens += _amount;
  ...
  uint256 feeEarlyUnstake;
    ...
    function _calculateRewards(address _user) private {
      ...
      _users[msg.sender].rewardAmount += userReward;
      _users[msg.sender].lastRewardCalculationTime = currentTime;
    }
